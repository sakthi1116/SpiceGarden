...
export default function AdminBilling() {
  ...
  useEffect(() => {
    const loadOrders = () => {
      ...
      if (savedOrders) {
        setBillsData(JSON.parse(savedOrders));
      } else {
  ...
  const filteredBills = billsData.filter(
    (bill) =>
      ...
      bill.table.toString().includes(searchQuery)
  );
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <button className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-amber-500 to-yellow-600 text-white rounded-xl hover:shadow-lg transition-all font-medium">
          <Download size={20} />
          Export Bills
        </button>
      </div>
          ...
          <tbody className="divide-y divide-gray-200">
            {filteredBills.map((bill) => (
              <tr key={bill.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <span className="text-gray-800">Table {bill.table}</span>
                </td>
                <td className="px-6 py-4">
